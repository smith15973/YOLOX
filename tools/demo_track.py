...
def imageflow_track_demo(predictor, vis_folder, current_time, args):
    ...
    while True:
        ...
        if outputs[0] is not None:
            ...
            if track_cls_id is not None:
                keep = (clses == track_cls_id)
                bboxes = bboxes[keep]
                scores = scores[keep]

            if len(bboxes) > 0:
                dets = np.concatenate([bboxes, scores.reshape(-1, 1)], axis=1).astype(np.float32)
                print("detections:", len(dets))


                # ByteTrack YOLOX wrapper typically wants:
                # update(dets, img_info, img_size)
                online_targets = tracker.update(
                    dets,
                    [img_info["height"], img_info["width"]],   # <- list/tuple, not dict
                    predictor.test_size,
                )
                print("tracks:", len(online_targets))
                # cv2.imshow("yolox_raw", predictor.visual(outputs[0], img_info, predictor.confthre))
        ...
        for t in online_targets:
            ...
            cv2.putText(
                result_frame, f"ID {tid}", (x1, max(0, y1 - 5)),
