...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="YOLOX image demo launcher (auto device).")
    ...
    for p in (demo_py, exp_file, ckpt_file, img_path):
        if not p.exists():
            print(f"ERROR: missing file: {p}")
            return 2

    cmd = [
        sys.executable, str(demo_py), "image",
        "-f", str(exp_file),
        "-c", str(ckpt_file),
        "--path", str(img_path),
        "--conf", str(args.conf),
