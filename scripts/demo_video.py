...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="YOLOX video demo launcher (auto device).")
    ...
    video_path = Path(args.path).expanduser()
    if not video_path.is_absolute():
        video_path = (Path.cwd() / video_path).resolve()

    for p in (demo_py, exp_file, ckpt_file, video_path):
        if not p.exists():
            print(f"ERROR: missing file: {p}")
            return 2

    cmd = [
        sys.executable, str(demo_py), "video",
        "-f", str(exp_file),
        "-c", str(ckpt_file),
        "--path", str(video_path),
        "--conf", str(args.conf),
